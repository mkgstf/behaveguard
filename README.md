...
- **Promotion gate**: the worker doesn't blindly overwrite the live model. It trains a candidate on a held-out split (most recent session held out per profile with ≥3 sessions), temperature-calibrates its probabilities, and compares balanced accuracy, macro-F1, then negative log loss with the active model. A worse candidate is kept as `status='candidate'` — visible via `GET /api/v1/admin/model-versions` — rather than silently replacing a working model.
...
Every enrollment updates the feature scaler, robust profile baselines, centroids, and RBF-SVM artifact. The classical verifier drops constant and explicit capture-length features, then calibrates a global operating threshold with profile-stratified held-session folds and identity-disjoint unknown probes. Per-profile thresholds are used only when enough genuine calibration trials exist.

Once there are at least two profiles with two independent sessions each and six eligible sessions overall, the background worker trains the BiLSTM keyboard + TCN mouse fusion model. Its scaler is fit only on the training partition, its loss is class-balanced, padded timesteps are mask-pooled, and mouse deltas are derived from absolute coordinates when needed. Older neural artifacts with incompatible sequence semantics are ignored until retrained.
